...
import { GraduationCap, BookOpen, Clock, Award, LogOut, Settings as SettingsIcon, ShieldCheck, Sparkles, RefreshCw } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
    ...
    try {
      ...
      if (courseData) {
        ...
        if (enrollmentData && enrollmentData.status === 'active') {
          ...
          setProgress(totalSections > 0 ? (completedSections / totalSections) * 100 : 0);
  ...
  const handleResetProgress = async () => {
    ...
    try {
      ...
      await supabase.from('exercise_submissions').delete().eq('user_id', user.id);
      await supabase.from('certificates').delete().eq('user_id', user.id);

      await loadDashboardData();

      alert('Course progress reset successfully! You can now start fresh from the beginning.');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative">
      ...
      <nav className={`bg-white/80 backdrop-blur-sm shadow-sm border-b border-slate-200 transition-opacity duration-500 relative z-10 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        ...
        {course ? (
          ...
          <div className={`bg-white rounded-3xl shadow-[0_2.8px_2.2px_rgba(0,_0,_0,_0.034),_0_6.7px_5.3px_rgba(0,_0,_0,_0.048),_0_12.5px_10px_rgba(0,_0,_0,_0.06),_0_22.3px_17.9px_rgba(0,_0,_0,_0.072),_0_41.8px_33.4px_rgba(0,_0,_0,_0.086),_0_100px_80px_rgba(0,_0,_0,_0.12)] ring-1 ring-black/5 overflow-hidden mb-8 p-12 text-center transition-all duration-700 delay-200 ${mounted ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-3">No Courses Available</h2>
            <p className="text-slate-600">Check back soon for new courses!</p>
          </div>
        )}

        <div className="grid md:grid-cols-3 gap-6">
          <div className={`bg-white rounded-2xl shadow-[0_2.8px_2.2px_rgba(0,_0,_0,_0.034),_0_6.7px_5.3px_rgba(0,_0,_0,_0.048),_0_12.5px_10px_rgba(0,_0,_0,_0.06),_0_22.3px_17.9px_rgba(0,_0,_0,_0.072),_0_41.8px_33.4px_rgba(0,_0,_0,_0.086),_0_100px_80px_rgba(0,_0,_0,_0.12)] ring-1 ring-black/5 p-6 transition-all duration-700 delay-300 hover:shadow-2xl hover:-translate-y-1 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-slate-900">Enrolled Courses</h3>
